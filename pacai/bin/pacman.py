...
class ClassicGameRules(object):
    ...
    def newGame(self, layout, pacmanAgent, display, catchExceptions = False):
        agents = [pacmanAgent]
...
def runGames(layout, pacman, display, numGames, record = None, numTraining = 0,
    ...
    for i in range(numGames):
        ...
        game = rules.newGame(layout, pacman, gameDisplay, catchExceptions)
        game.run()
    ...
    if ((numGames - numTraining) > 0):
        ...
        average = 'Average Score: ' + str(sum(scores) / float(len(scores)))
        scores = 'Scores:        ' + ', '.join([str(score) for score in scores])
        winRate = 'Win Rate:      ' + str(winRate)
        record = 'Record:        ' + ', '.join([['Loss', 'Win'][int(w)] for w in wins])
        logging.info(average)
        logging.info(scores)
        logging.info(winRate)
        logging.info(record)
